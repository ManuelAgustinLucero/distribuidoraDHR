...
class ProductoController extends Controller
{
    ...
    public function cargarAction(Request $request)
    {
        ...
        $idArray= $request->request->get('idArray');

        $valor=floatval($valor);
        $em = $this->getDoctrine()->getManager();
        foreach ($idArray as $id){
            $db = $em->getConnection();
            $query ="UPDATE producto SET precio=producto.precio+ '$valor' where producto.id=$id";
        ...
        foreach ($idArray as $id){
            $db = $em->getConnection();
            $query ="UPDATE cliente_producto SET precio=cliente_producto.precio+ '$valor' where cliente_producto.producto_id=$id";
            $stmt = $db->prepare($query);
            $params = array();
            $stmt->execute($params);
        }
